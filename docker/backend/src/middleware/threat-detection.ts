...
const THREAT_INTELLIGENCE = {
  ...
  suspiciousUserAgents: new Set<string>(),
  recentAttacks: new Map<string, number>()
};
...
function analyzeRequest(req: Request): ThreatDetectionResult {
  let totalScore = 0;
  ...
  let recommendedAction: 'allow' | 'block' | 'monitor' | 'challenge';
...
// Threat detection middleware
export const threatDetection = (req: Request, res: Response, next: NextFunction): void => {
  try {
    ...
    if (analysis.isThreat) {
      ...
      if (analysis.recommendedAction === 'block') {
        return next(createError('Access denied due to security policy', 403, 'ACCESS_DENIED'));
      }

      // Add threat information to request for monitoring
      (req as any).threatAnalysis = analysis;
    }

    next();
  } catch (error) {
    LoggerService.error('Error in threat detection', { error, ip: req.ip, url: req.url });
...
export const behavioralAnalysis = (req: Request, res: Response, next: NextFunction): void => {
  ...
  if (recentRequests > 100) { // More than 100 requests in tracking window
    ...
    return next(createError('Too many requests', 429, 'RATE_LIMIT_EXCEEDED'));
  }

  // Check if IP is blocked
  if (THREAT_INTELLIGENCE.blockedIPs.has(clientIP)) {
    LoggerService.warn('Request from blocked IP', {
      ip: clientIP,
      url: req.url
    });
...
export const clearThreatIntelligence = () => {
